...
// V29 test database layout and v29 upgrade test plan
//
// The v29 database upgrade introduces the accountconsolidation bucket to store
// per-account consolidation addresses for SSFee UTXO batching. This test database
// generates votes WITH consolidation outputs to verify that the consolidation
// feature works correctly and produces expected balance calculations.
//
// The v29 database generated by this file is meant to be used on the v29
// upgrade verification test (verifyV29Upgrade). This database is setup with a
// set of tickets, votes and revocations with MODERN vote format (including
// consolidation outputs) to test that:
// 1. The upgrade to v29 succeeds
// 2. Votes with consolidation outputs properly mark ticket commitments as spent
// 3. Balance calculations show locked:0 instead of locked:1000 for voted tickets
//
...
func setup() error {
	...
	defer db.Close()
	var seed [32]byte
	err = udb.Initialize(ctx, db, chainParams, seed[:], pubPass, privPass)
	if err != nil {
		return err
	}

	amgr, txmgr, _, err := udb.Open(ctx, db, chainParams, pubPass)
	if err != nil {
		return err
	}
